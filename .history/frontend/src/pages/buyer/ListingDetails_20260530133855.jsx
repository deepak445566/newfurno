...
const ListingDetails = () => {
  ...
  const checkRequestStatus = async (listingId) => {
    try {
      const res = await api.get(`/api/requests/check/${listingId}`, { withCredentials: true });
  ...
  const handleRequest = async () => {
    ...
    try {
      const response = await api.post("/api/requests/create", {
        ...
        message: `I'm interested in your item: ${listing.title}`
  ...
  const handleMessage = async () => {
    ...
    try {
      ...
      const checkResponse = await api.post("/api/chat/check", {
        listingId: listing._id,
        userId: listing.userId._id
      }, { withCredentials: true });
      
      let chatId = checkResponse.data.chatId;
      
      // If chat doesn't exist, create one
      if (!chatId) {
        const createResponse = await api.post("/api/chat/create", {
          listingId: listing._id,
          participants: [currentUser._id, listing.userId._id],
          initialMessage: `Hi! I'm interested in your item: ${listing.title}`
        }, { withCredentials: true });
    ...
    return (
      <div className="min-h-screen bg-[#f5f5f7] flex items-center justify-center">
        <div className="text-center">
          ...
          <button onClick={() => navigate("/listings")} className="mt-4 px-6 py-3 bg-black text-white rounded-2xl">
            Back
          </button>
        </div>
      </div>
  ...
  const isOwner = currentUser && listing.userId && currentUser._id === listing.userId._id;
    ...
    <div className="min-h-screen bg-[#f5f5f7] py-4 sm:py-10 px-3 sm:px-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-5 sm:mb-8 gap-3">
          <button onClick={() => navigate(-1)} className="group flex items-center gap-2 sm:gap-3 bg-white border border-gray-200 px-3 sm:px-5 py-2.5 sm:py-3 rounded-2xl hover:shadow-lg transition-all duration-300">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1.1fr_0.9fr] gap-5 sm:gap-8">
          {/* LEFT SIDE - Image section (same as before) */}
          <div>
            <div className="relative overflow-hidden rounded-[22px] sm:rounded-[36px] bg-white border border-gray-200 shadow-[0_20px_60px_rgba(0,0,0,0.08)]">
              {listing.images && listing.images.length > 0 ? (
                <img src={listing.images[selectedImage]} alt={listing.title} className="w-full h-[300px] sm:h-[500px] lg:h-[620px] object-cover sm:object-contain" />
              ) : (
              ...
              <div className="absolute bottom-4 sm:bottom-8 left-4 sm:left-8">
                <p className="text-white/70 uppercase tracking-[4px] text-[10px] sm:text-xs">Price</p>
            ...
            {listing.images && listing.images.length > 1 && (
              <div className="flex gap-3 sm:gap-4 mt-4 sm:mt-5 overflow-x-auto pb-2">
                {listing.images.map((img, index) => (
                  <button key={index} onClick={() => setSelectedImage(index)} className={`relative overflow-hidden rounded-2xl border-2 transition-all duration-300 flex-shrink-0 ${
                    selectedImage === index ? "border-black scale-105" : "border-gray-200 hover:border-gray-400"
                  }`}>
                    <img src={img} alt="" className="w-16 h-16 sm:w-24 sm:h-24 object-cover" />
                  </button>
                ))}
              </div>
            )}
          ...
          <div className="space-y-4 sm:space-y-6">
            ...
            {/* DESCRIPTION */}
            <div className="bg-white rounded-[22px] sm:rounded-[32px] border border-gray-200 p-4 sm:p-8 shadow-sm">
              <h2 className="text-lg sm:text-xl font-bold text-gray-900 mb-5">Description</h2>
            ...
            {/* SELLER */}
            <div className="bg-white rounded-[22px] sm:rounded-[32px] border border-gray-200 p-4 sm:p-8 shadow-sm">
              <h2 className="text-lg sm:text-xl font-bold text-gray-900 mb-6">Seller Information</h2>
              <div className="flex items-center gap-4 sm:gap-5">
                ...
                <div>
                  ...
                  <p className="text-gray-500 text-sm mt-1">Trusted Seller</p>
                </div>
              </div>

              {!showContact ? (
                <button onClick={() => setShowContact(true)} className="w-full mt-5 sm:mt-7 h-12 sm:h-14 rounded-2xl bg-black text-white font-semibold hover:scale-[1.01] transition-all duration-300">
                  📞 Contact Seller
                </button>
              ) : (
                <div className="mt-5 rounded-2xl bg-gray-50 border border-gray-200 p-4">
                  <div className="space-y-3">
                    <div className="text-sm sm:text-base text-gray-700">📞 {listing.userId?.phone || "Not Provided"}</div>
                    <div className="text-sm sm:text-base text-gray-700">📧 {listing.userId?.email || "Not Provided"}</div>
                  </div>
                </div>
              )}
            </div>
            ...
            {/* LOCATION */}
            <div className="bg-white rounded-[22px] sm:rounded-[32px] border border-gray-200 p-4 sm:p-8 shadow-sm">
              <h2 className="text-lg sm:text-xl font-bold text-gray-900 mb-5">Pickup Location</h2>
              <div className="flex items-start gap-3 sm:gap-4">
                <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-gray-100 flex items-center justify-center text-xl sm:text-2xl shrink-0">📍</div>
                <div>
                  <p className="text-gray-700 text-sm sm:text-base leading-relaxed">{listing.address}</p>
                  <button className="mt-4 px-5 py-2 rounded-full bg-black text-white text-sm hover:bg-gray-800 transition">
                    View on Map
                  </button>
                </div>
              </div>
            ...
            {/* ACTION BUTTONS - Updated with working buttons */}
            <div className="grid grid-cols-2 gap-3 sm:gap-4">
              {/* MESSAGE BUTTON - Working */}
              <button 
                onClick={handleMessage}
                disabled={requestLoading || isOwner}
                className={`h-12 sm:h-14 rounded-2xl font-semibold transition-all duration-300 flex items-center justify-center gap-2 ${
                  isOwner 
              ...
              {/* REQUEST BUTTON - Added */}
              <button 
                onClick={handleRequest}
                disabled={requestLoading || isOwner || hasRequested}
                className={`h-12 sm:h-14 rounded-2xl font-semibold transition-all duration-300 flex items-center justify-center gap-2 ${
                  isOwner 
                    ? "bg-gray-300 cursor-not-allowed text-gray-600" 
                    : hasRequested
                    ? "bg-green-100 text-green-700 cursor-default border border-green-300"
                    : "border-2 border-black bg-white text-black hover:bg-black hover:text-white active:scale-95"
                }`}
              >
                {requestLoading ? (
